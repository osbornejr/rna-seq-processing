...
rule trinity_normalisation:
	input: 
		left=expand("clean-reads/{sample}/{sample}_read_1_fastp.fastq.gz",sample=config["samples"]),
                right=expand("clean-reads/{sample}/{sample}_read_2_fastp.fastq.gz",sample=config["samples"])
	output: 
